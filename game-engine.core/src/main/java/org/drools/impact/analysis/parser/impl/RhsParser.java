...
/*
 * SHADOW PATCH of drools-impact-analysis-parser 10.1.0.
 * Verbatim copy of org.drools.impact.analysis.parser.impl.RhsParser with one behavioural fix:
 * when an insert/delete action type can't be resolved to a real Class — e.g.
 * `insert(new BadgeUpdate(...))` where BadgeUpdate is a `declare`d type with no compiled class
 * on the analysis classpath — upstream throws RuntimeException(ClassNotFoundException) and aborts
 * the whole game's analysis. Here getActionClass returns null instead, and processInsert/processAction
 * drop the action (the pipeline already filters nulls), so the rest of the rules still analyze.
 * Shadows the jar via classpath precedence. Remove once fixed upstream (apache/incubator-kie-drools).
 */
...
public class RhsParser {
    ...
    private ConsequenceAction processAction( RuleContext context, MethodCallExpr consequenceExpr, MethodCallExpr statement, ConsequenceAction.Type type ) {
        ...
        if (actionClass == null) {
            return null; // SHADOW PATCH: unresolved action type — drop action instead of failing the build
        }
    ...
    private Class<?> getActionClass(RuleContext context, MethodCallExpr consequenceExpr, MethodCallExpr statement) {
        ...
        if (actionArg.isNameExpr()) {
            ...
            try {
                ...
                // SHADOW PATCH: declared types (declare X) aren't loadable classes during analysis — skip
                return null;
            }
        }
        return actionClass;
    ...
    private Class<?> getClassFromAssignment( MethodCallExpr consequenceExpr, Expression actionArg ) {
        ...
        try {
            ...
            // SHADOW PATCH: unresolvable type — skip instead of failing the build
            return null;
        }
    }
    ...
    private InsertAction processInsert( RuleContext context, MethodCallExpr consequenceExpr, MethodCallExpr statement, BlockStmt ruleVariablesBlock ) {
        Class<?> actionClass = getActionClass(context, consequenceExpr, statement);
        if (actionClass == null) {
            return null; // SHADOW PATCH: unresolved insert type (e.g. declared type) — drop action, don't crash
        }
