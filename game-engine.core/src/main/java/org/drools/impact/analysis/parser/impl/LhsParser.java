 ...
 * Verbatim copy of org.drools.impact.analysis.parser.impl.LhsParser with a single fix in
 * processMapProperty: strip enclosing casts before asMethodCallExpr(). Upstream's
 * parseExpressionInConstraint strips the cast into a local var but then re-reads the
 * original (unstripped) expression in processMapProperty, throwing IllegalStateException
 * on constraints like `badgeEarned.size() >= (Double)$x` (codegen inserts a (double) cast).
 * This class shadows the jar version via classpath precedence (project classes before deps).
 * Remove once fixed upstream (apache/incubator-kie-drools).
 */
...
public class LhsParser {
    ...
    private Constraint processMapProperty(RuleContext context, Constraint constraint, TypedExpression expr) {
        // SHADOW PATCH: strip enclosing cast before asMethodCallExpr() — upstream uses the raw expression here
        MethodCallExpr mce = stripEnclosedAndCast(expr.getExpression()).asMethodCallExpr();
